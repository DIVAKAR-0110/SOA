...
const DUMMY_COMPLAINTS = [
  { id: 1, title: 'Pothole on Main St', category: 'Roads', location: 'Sector 5', status: 'REGISTERED', registeredAt: '2025-12-25T10:30:00' },
  { id: 2, title: 'Streetlight not working', category: 'Utilities', city: 'Block A', status: 'IN_PROGRESS', registeredAt: '2025-11-18T09:15:00' },
  { id: 3, title: 'Garbage collection missed', category: 'Sanitation', location: 'Zone 3', status: 'RESOLVED', registeredAt: '2025-10-05T14:45:00' },
  { id: 4, title: 'Water leakage', category: 'Water', location: 'Market Area', status: 'IN-PROGRESS', registeredAt: '2025-12-01T08:20:00' },
  { id: 5, title: 'Tree fallen', category: 'Parks', location: 'Riverside', status: 'registered', registeredAt: '2025-12-20T16:00:00' },
];
...
function MyComplaints() {
  const [complaints, setComplaints] = useState(() => {
    try {
      ...
      for (const c of combined) {
        ...
        if (!seen.has(key)) { seen.set(key, true); result.push(c); }
      }
      return result;
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const storageHandler = (e) => { if (e.key === 'my_complaints') handler(); };
    window.addEventListener('storage', storageHandler);
    return () => { window.removeEventListener('mycomplaints:updated', handler); window.removeEventListener('storage', storageHandler); };
  }, []);
