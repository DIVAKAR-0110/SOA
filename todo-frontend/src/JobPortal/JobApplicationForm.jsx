...
export default function JobApplicationForm() {
  ...
  const normalizeOptions = (items) =>
    Array.isArray(items)
      ? items.map((it) => ({
          id:
            it.skill_id ?? // ✅ ADD THIS LINE (TOP PRIORITY)
            it.department_id ??
            it.category_id ??
            it.id ??
            it.country_id ??
  ...
  const fetchSkills = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5000/api/jobapplication-skills",
      );
      setSkills(normalizeOptions(res.data));
    } catch (err) {
  ...
  const handleSkill = async (id) => {
    setForm({ ...form, skill_id: id, selected_skill: "" });
    fetchSkills(id);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!form.role_applied_for) {
        alert("Please select the role you are applying for.");
        setSubmitting(false);
        return;
      }

      if (form.role_applied_for === "Driver") {
        if (!form.vehicle_available) {
          alert("Drivers must indicate vehicle availability.");
          setSubmitting(false);
          return;
        }
        if (!form.driving_license_no) {
          alert("Drivers must provide a driving license number.");
          setSubmitting(false);
          return;
        }
      }
      ...
      if (form.role_applied_for === "Employee") {
        if (!form.interested_department) {
          alert("Employee must select a department.");
          setSubmitting(false);
          return;
        }
        if (!form.selected_category) {
          alert("Employee must select a category.");
          setSubmitting(false);
          return;
        }
        if (!form.selected_skill) {
          alert("Employee must select a skill.");
          setSubmitting(false);
          return;
        }
      }
      ...
      if (degreeFile) formData.append("degree_certificate", degreeFile);
        ...
        <>
          ...
          <div className="form-row">
            ...
            <Dropdown
              ...
              options={skills}
              onChange={(id) => setForm({ ...form, selected_skill: id })}
              disabled={!form.interested_department || lockFields}
            />
