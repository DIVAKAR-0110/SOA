...
import { useLanguage } from "../../Admin/LanguageContext/LanguageContext.jsx";
import { getCategoryTitle, formLabels } from "../../Components/utils/i18n.js";
import UserSidebar from "../../Admin/UserSidebar/UserSidebar.jsx";
...
function ComplaintForm({ initialCategory = "" }) {
  ...
  const speakPrompt = useCallback(
    (fieldName, onPromptEnd) => {
      try {
        ...
        const prompt =
          labels.voicePrompts?.[fieldName] ||
          `Please provide input for ${fieldName}`;
  ...
  const handleTranscript = useCallback((name, text) => {
    ...
    if (name === "description") {
      // Simply append spoken text
      setFormData((prev) => ({
        ...prev,
        description: prev.description
          ? prev.description + " " + cleaned
          : cleaned,
      }));
      return;
    }

    // Example: matching numbers in Tamil / Hindi / English
    if (name === "priority") {
      const map = { normal: "Normal", high: "High", emergency: "Emergency" };
      const key = Object.keys(map).find(
        (k) =>
          cleaned.toLowerCase().includes(k) ||
          k.includes(cleaned.toLowerCase()),
      );
    ...
    // Example for city or numbers using variants
    if (name === "city") {
      const options = ["City A", "City B", "Zone 1"];
      const found = options.find(
        (opt) =>
          opt.toLowerCase().includes(cleaned.toLowerCase()) ||
  ...
  function matchVariant(input, variantsList) {
    const normalized = input.trim().toLowerCase();
    for (const variants of variantsList) {
      for (const variant of variants) {
        if (normalized === variant.toLowerCase()) return variant; // exact match
        if (normalized.includes(variant.toLowerCase())) return variant; // partial match
      }
    }
    return null;
  }
  ...
  useEffect(() => {
    if (initialCategory)
      setFormData((s) => ({ ...s, category: initialCategory }));
  }, [initialCategory]);
  ...
  const validate = () => {
    ...
    if (countWords(formData.description) > maxWords)
      errs.description = labels.errors.descriptionTooLong(maxWords);
    setErrors(errs);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errMsg =
          body.error || body.message || res.statusText || "Failed to submit";
        throw new Error(errMsg);
      }

      alert(body.message || labels.success.registered(body?.id));

      // persist a copy locally (frontend-only) so it appears on the My Complaints page immediately
      try {
        const newComplaint = {
          id: body?.id ?? Date.now(),
          ...
          status: (body?.status ?? "REGISTERED").toString(),
        ...
        <div className="form-title-area">
          ...
          {displayedCategory && (
            <div className="category-pill">
              {getCategoryTitle(displayedCategory, language)}
            </div>
          )}
        </div>
      </div>
      ...
      <form className="complaint-form" onSubmit={handleSubmit}>
        ...
        <div className="form-card">
          ...
          <div className="two-col">
            <div>
              <label>
                {labels.cityLabel} <span className="required">*</span>
              </label>
              <div className="input-with-voice">
                ...
                <button
                  type="button"
                  className={`voice-btn ${listeningField === "city" ? "listening" : ""}`}
                  onClick={() =>
                    listeningField === "city"
                      ? stopListening()
                      : startListening("city")
                  }
                  aria-pressed={listeningField === "city"}
                  aria-label={
                    listeningField === "city"
                      ? "Stop listening"
                      : "Record city via voice"
                  }
                >
            ...
            <div>
              <label>
                {labels.streetLabel} <span className="required">*</span>
              </label>
              <div className="input-with-voice">
                <input
                  name="street"
                ...
                <button
                  type="button"
                  className={`voice-btn ${listeningField === "street" ? "listening" : ""}`}
                  onClick={() =>
                    listeningField === "street"
                      ? stopListening()
                      : startListening("street")
                  }
                  aria-pressed={listeningField === "street"}
                  aria-label={
                    listeningField === "street"
                      ? "Stop listening"
                      : "Record street via voice"
                  }
                >
          ...
          <label>
            {labels.addressLabel} <span className="required">*</span>
          </label>
          <div className="input-with-voice">
            <input
              name="address"
            ...
            <button
              type="button"
              className={`voice-btn ${listeningField === "address" ? "listening" : ""}`}
              onClick={() =>
                listeningField === "address"
                  ? stopListening()
                  : startListening("address")
              }
              aria-pressed={listeningField === "address"}
              aria-label={
                listeningField === "address"
                  ? "Stop listening"
                  : "Record address via voice"
              }
            >
          ...
          <div className="input-with-voice">
            ...
            <button
              type="button"
              className={`voice-btn ${listeningField === "landmark" ? "listening" : ""}`}
              onClick={() =>
                listeningField === "landmark"
                  ? stopListening()
                  : startListening("landmark")
              }
              aria-pressed={listeningField === "landmark"}
              aria-label={
                listeningField === "landmark"
                  ? "Stop listening"
                  : "Record landmark via voice"
              }
            >
        ...
        <div className="form-card">
          ...
          <label>
            {labels.complaintTitle} <span className="required">*</span>
          </label>
          <div className="input-with-voice">
            <input
              name="title"
            ...
            <button
              type="button"
              className={`voice-btn ${listeningField === "title" ? "listening" : ""}`}
              onClick={() =>
          ...
          <label>
            {labels.complaintDescription} <span className="required">*</span>
          </label>
          <div
          ...
          >
            ...
            <button
              type="button"
              className={`voice-btn ${listeningField === "description" ? "listening" : ""}`}
              onClick={() =>
          ...
          <div className="help">
            {labels.helpWords(countWords(formData.description), maxWords)}
          </div>
          {lastTranscript && (
          ...
          <div className="two-col">
            <div>
              ...
              <div className="input-with-voice">
                ...
                >
                  <option value="Normal">
                    {labels.priorityOptions.Normal}
                  </option>
                  <option value="High">{labels.priorityOptions.High}</option>
                  <option value="Emergency">
                    {labels.priorityOptions.Emergency}
                  </option>
                </select>
                <button
                  type="button"
                  className={`voice-btn ${listeningField === "priority" ? "listening" : ""}`}
                  onClick={() =>
                    listeningField === "priority"
                      ? stopListening()
                      : startListening("priority")
                  }
                  aria-pressed={listeningField === "priority"}
                  aria-label={
                    listeningField === "priority"
                      ? "Stop listening"
                      : "Record priority via voice"
                  }
                >
              ...
              <div className="input-with-voice">
                ...
                >
                  <option value="Morning">
                    {labels.contactOptions.Morning}
                  </option>
                  <option value="Afternoon">
                    {labels.contactOptions.Afternoon}
                  </option>
                  <option value="Evening">
                    {labels.contactOptions.Evening}
                  </option>
                </select>
                <button
                  type="button"
                  className={`voice-btn ${listeningField === "contact_time" ? "listening" : ""}`}
                  onClick={() =>
                    listeningField === "contact_time"
                      ? stopListening()
                      : startListening("contact_time")
                  }
                  aria-pressed={listeningField === "contact_time"}
                  aria-label={
                    listeningField === "contact_time"
                      ? "Stop listening"
                      : "Record preferred contact time via voice"
                  }
                >
  ...
  const isStandalone = !location.state?.fromDashboard;
  ...
  if (isStandalone) {
    return (
      <div className="user-dashboard-wrapper">
        ...
        />
        <div className="dashboard-container">{formContent}</div>
      </div>
    );
  }
