...
import i18n, {
  numberWords,
  categories as i18nCategories,
  numberToCategoryId,
  getCategoryTitle,
  getCategoryDescription,
} from "../../Components/utils/i18n.js";
import UserSidebar from "../../Admin/UserSidebar/UserSidebar.jsx";

const languagePrompts = {
  "en-IN": {
    label: "English",
    prompt: "Please say the category number from one to eight.",
  },
  "ta-IN": {
    label: "Tamil",
    prompt: "தயவு செய்து ஒரு முதல் எட்டு வரை பிரிவு எண் சொல்லுங்கள்.",
  },
  "hi-IN": {
    label: "Hindi",
    prompt: "कृपया एक से आठ के बीच श्रेणी संख्या बोलें।",
  },
};
...
function extractNumberFromText(text) {
  ...
  for (const t of tokens) {
    if (tokenToNumber[t] && typeof tokenToNumber[t] === "number")
      return tokenToNumber[t];
  }

  // fallback: exact match
  if (
    tokenToNumber[normalized] &&
    typeof tokenToNumber[normalized] === "number"
  )
    return tokenToNumber[normalized];
...
function CategoryPage({ onSelectCategory }) {
  ...
  const handleApply = (cat) => {
    ...
    navigate("/complaint/new", { state: { category: cat } });
  };
  ...
  const startRecognition = () => {
    ...
    if (!SpeechRecognition) {
      alert(
        "Speech Recognition not supported. Please use Chrome or an equivalent browser.",
      );
      return;
    }
    ...
    try {
        ...
        if (final.trim()) {
          ...
          if (num && num >= 1 && num <= 8) {
            ...
            const confirmUtter = new SpeechSynthesisUtterance(
              confirmPhrases[language] || confirmPhrases["en-IN"],
            );
      ...
      rec.onerror = (e) => {
        ...
        if (e && e.error === "not-allowed")
          setStatus(
            "Microphone permission denied. Allow microphone and try again.",
          );
        else
          setStatus("Error during recognition. Speak clearly and try again.");
      };
  ...
  const categoryContent = (
    <div className="category-page">
      ...
      >
        ...
        <button onClick={startListening} className="apply-btn">
          🎙 Start
        </button>
        <button onClick={readAndAsk} className="apply-btn">
          🔈 Read & Ask
        </button>
        <button
      ...
      {pendingCategory && (
        ...
        >
          <div>
            Detected category:{" "}
            <strong>{getCategoryTitle(pendingCategory, language)}</strong> (
            {numberWords[language]
              ? numberWords[language][pendingNumber - 1]
              : `#${pendingNumber}`}
            )
          </div>
          <div style={{ marginTop: 8 }}>
            <button className="apply-btn" onClick={confirmSelection}>
              Confirm
            </button>
            <button
              className="apply-btn"
        ...
        {categories.map((cat, idx) => (
          <div key={cat.id} className="category-card">
            <div className="category-card-body">
              <h3>
                {(numberWords[language]
                  ? numberWords[language][idx] + " - "
                  : idx + 1 + " - ") + localizedTitle(cat, idx, language)}
              </h3>
  ...
  if (isStandalone) {
    return (
      <div className="user-dashboard-wrapper">
        ...
        />
        <div className="dashboard-container">{categoryContent}</div>
      </div>
    );
  }
